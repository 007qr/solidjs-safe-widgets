...
const OTPInput: Component<OTPInputProps> = (props) => {
  ...
  const [otpValues, setOtpValues] = createSignal<string[]>(Array(props.length).fill(""));
  ...
  return (
    <div class={`flex gap-[8px] ${props.className || ""}`} style={props.style}>
      <For each={Array(props.length).fill(0)}>
        {(_, index) => (
          <input
            ...
            class={`w-12 h-12 text-center border rounded ${props.error ? "border-red-500" : "border-gray-300"} focus:outline-none focus:ring-2 focus:ring-blue-500 ${props.inputClassName || ""}`}
